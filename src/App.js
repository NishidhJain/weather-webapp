...
function App() {
  ...
  const findCity = "https://api.openweathermap.org/data/2.5/weather?";
  ...
  //       const findCityURL = `${findCity}lat=${latitude}&lon=${longitude}&appid=${process.env.REACT_APP_API_KEY}`;
  //       try {
  //         const currentCityURL = `${oneCallAPI_URL}?lat=${latitude}&lon=${longitude}&exclude=minutely,hourly,alerts&units=metric&appid=${process.env.REACT_APP_API_KEY}`;
  //         console.log(currentCityURL);
  //         const currentCityData = await fetch(currentCityURL);
  //         const currentCityJSON = await currentCityData.json();
  //         console.log("Current location data: ", currentCityJSON);
  //       } catch (err) {
  //         console.log("Err in fetching current city data: ", err);
  //       }
  //     });
  //   } else {
  ...
  const getCurrentLocationData = () => {
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(async ({ coords }) => {
        ...
        const findCityURL = `${findCity}lat=${latitude}&lon=${longitude}&units=metric&appid=${process.env.REACT_APP_API_KEY}`;

        try {
          const cityDetails = await fetch(findCityURL);
          const cityDetailsJSON = await cityDetails.json();
          // console.log(cityDetailsJSON);
          setData(cityDetailsJSON);

          // if data is found then fetch forecast data
          if (cityDetailsJSON.cod == "200") {
            const oneAPI = `${oneCallAPI_URL}?lat=${latitude}&lon=${longitude}&exclude=minutely,hourly,alerts&units=metric&appid=${process.env.REACT_APP_API_KEY}`;

            const forecastData = await fetch(oneAPI);
          ...
          console.log("Err in fetching current city details: ", err);
        }
      });
    } else {
        ...
        <div className="app__top">
          ...
          {error ? (
            <div className="networkErr error__container">
              ...
              <p className="networkErr__warning">
          ...
          >
            <path
              fill="#fff"
              fillOpacity="1"
              d="M0,64L34.3,101.3C68.6,139,137,213,206,250.7C274.3,288,343,288,411,256C480,224,549,160,617,154.7C685.7,149,754,203,823,234.7C891.4,267,960,277,1029,250.7C1097.1,224,1166,160,1234,160C1302.9,160,1371,224,1406,256L1440,288L1440,320L1405.7,320C1371.4,320,1303,320,1234,320C1165.7,320,1097,320,1029,320C960,320,891,320,823,320C754.3,320,686,320,617,320C548.6,320,480,320,411,320C342.9,320,274,320,206,320C137.1,320,69,320,34,320L0,320Z"
            ></path>
          </svg>
